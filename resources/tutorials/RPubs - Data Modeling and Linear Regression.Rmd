...
ggplot(road, aes(Age.of.Casualty)) + # Initialise plot and set the x-axis to casualty age
    geom_histogram(binwidth=1, fill="#fddf00", alpha=0.75) + # Set width, colour and size
...
But what if we want to find out the significance of other variables? Let's do that here with a multiple linear regression model
```{r}
...
To test the significance of all variables, a shorter version of the code can be used:
```{r}
...
Ok, now I'm going to create a model with Number of Casualties as the dependent variable so we can compare the two
```{r}
...
Using multiplot, we can compare the two models - one where the dependent variable is Casualty Severity (linear_model_2) and the other Number of Casualties (linear_model_3). Each coefficient is plotted as a point with a thick line representing the one standard error confidence interval and a thin line representing the two standard error confidence interval. If the two standard error confidence interval does not contain 0 it is not statistically significant
...
## Summary
That's it! We've created a quick chart, found out the correlation between variables and performed multiple linear regression on the road casualty dataset. We've found out that some factors are statiscially significant when it comes to casualty severity and made some predictions.
